...
export default function ChordProgressionBuilder() {
  ...
  function setChordBeats(sectionId: string, index: number, beats: number) {
    updateSection(sectionId, (sec) => ({
      ...sec,
      progression: sec.progression.map((slot, i) =>
        i === index ? { ...slot, beats } : slot
      ),
    }))
  }

  function setChordQuality(sectionId: string, index: number, quality: string) {
    updateSection(sectionId, (sec) => ({
      ...sec,
      progression: sec.progression.map((slot, i) =>
        i === index ? { ...slot, chord: { ...slot.chord, quality } } : slot
      ),
    }))
  }
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={`flex w-80 shrink-0 flex-col overflow-y-auto border-r border-border bg-sidebar transition-transform md:relative md:flex ${
          ...
          <div className="flex min-h-full flex-col gap-5 p-5 pt-3 md:pt-5">
            ...
            {/* Controls help */}
            <details className="group list-none rounded-lg border border-border/40 bg-muted/10 p-2.5 [&::-webkit-details-marker]:hidden">
              <summary className="cursor-pointer text-xs font-medium text-muted-foreground transition-colors hover:text-foreground [&::-webkit-details-marker]:hidden">
            ...
            {song.sections.map((section, index) => (
              ...
              >
                <SectionBlock
                  ...
                  onBeatsChange={(index, beats) => setChordBeats(section.id, index, beats)}
                  onQualityChange={(index, quality) => setChordQuality(section.id, index, quality)}
